  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-0 sm:px-6 pt-0 sm:pt-4 pointer-events-none transition-all duration-500">
      <div className="max-w-7xl mx-auto flex items-center justify-start relative pointer-events-auto">
        ...
        <div
          className={`transition-all duration-500 ease-in-out flex items-center justify-between gap-2 sm:gap-4 overflow-hidden w-full ${isExpanded
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Link href="/" className="flex items-center gap-2.5 group shrink-0">
              <div className="relative">
                <div
                  className={`rounded-xl flex items-center justify-center text-white shadow-md transition-all duration-500 ${isScrolled
              ...
              <div>
                <div
                  className={`font-bold tracking-tight flex items-center gap-1.5 transition-all duration-500 ${isScrolled ? 'text-sm sm:text-xs text-white' : 'text-base sm:text-lg text-slate-900'
                    }`}
                >
                  DrivePulse{' '}
          ...
          {isExpanded && (
            <nav
              className={`hidden lg:flex items-center gap-1 text-xs font-semibold p-1 rounded-full border transition-all duration-500 animate-in fade-in duration-300 ${isScrolled
                ? 'bg-white/10 text-slate-100 border-white/15'
                : 'bg-primary text-white border-slate-200/70'
                }`}
            >
              <a
                href="#services"
                onClick={() => setScrolledMenuOpen(false)}
                className={`px-3.5 py-1.5 rounded-full transition-all duration-200 ${isScrolled
              ...
              <a
                href="#calculator"
                onClick={() => setScrolledMenuOpen(false)}
                className={`px-3.5 py-1.5 rounded-full transition-all duration-200 flex items-center gap-1.5 ${isScrolled
                  ? 'hover:text-white hover:bg-white/10'
                  : 'hover:text-[#023526] hover:bg-white hover:shadow-2xs'
                  }`}
              >
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            {isExpanded && (
              <a
                ...
                className={`hidden lg:flex items-center gap-1.5 text-[11px] font-bold px-3 py-2 rounded-xl border transition-all ${isScrolled
                  ? 'text-[#023526] bg-[#c5a880] hover:bg-[#b8996f] border-[#c5a880]'
                  : 'text-slate-700 bg-brand-gold hover:bg-slate-200/70 border-slate-200'
                  }`}
              >
                <Phone className={`w-3.5 h-3.5 ${isScrolled ? 'text-[#023526]' : 'text-black'}`} />
            ...
            <button
              ...
              className={`hidden sm:relative sm:group overflow-hidden rounded-xl p-px font-bold text-xs cursor-pointer transition-all duration-500 active:scale-95 ${!isExpanded
                ? 'rounded-full shadow-md'
                : 'shadow-[0_4px_20px_rgba(2,53,38,0.25)] hover:shadow-[0_6px_25px_rgba(2,53,38,0.4)]'
                }`}
            >
              <span
                className={`absolute inset-0 transition-opacity duration-500 group-hover:opacity-90 ${isScrolled
                  ? 'bg-gradient-to-r from-[#d4af37] via-[#f7e4af] to-[#9c7f56]'
                  : 'bg-gradient-to-r from-[#023526] via-[#c5a880] to-[#023526]'
                  }`}
              ></span>
              <span
                className={`relative flex items-center gap-1.5 transition-all duration-500 ${!isExpanded
            ...
            <button
              ...
              className={`sm:hidden p-2 rounded-xl border transition-colors focus:outline-none flex items-center justify-center ${isScrolled
        ...
        {mobileMenuOpen && (
          <div
            className={`w-full backdrop-blur-xl border-x border-b sm:border rounded-b-2xl sm:rounded-2xl p-4 mt-0 sm:mt-2 shadow-2xl space-y-2.5 animate-in fade-in zoom-in-95 duration-200 pointer-events-auto ${isScrolled
